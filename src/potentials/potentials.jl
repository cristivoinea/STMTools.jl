...
function get_pseudopotentials(m_max, beta, eps, nbr_g, d_g; rpa=false, nuN=[], alpha=1.85)
    v0 = if nbr_g == 0
...
function get_impurity_potentials(m_max, beta, eps, nbr_g, d_g, q_i, d_i; rpa=false, nuN=[], alpha=1.85)
    v0 = if nbr_g == 0
...
function get_tip_potentials(nm, thetas, beta, eps, nbr_g, d_g, q_i, d_i, r_t, d_t; rpa=false, nuN=[], alpha=1.85)
    v0_imp = if nbr_g == 0
        q -> v_impurity_bare_onebody(q, beta, eps, q_i, d_i)
    elseif nbr_g == 1
        q -> v_impurity_singlegate_onebody(q, beta, eps, q_i, d_i, d_g)
